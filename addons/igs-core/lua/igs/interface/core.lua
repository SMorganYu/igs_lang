...
function IGS.UI()
	if not IGS.IsLoaded() then
		LocalPlayer():ChatPrint(IGS.GetPhrase("autodonatenotloaded"))
		return
	end

	if not IGS.C then -- Проблема AddCSLua. В консоли клиента должны быть ошибки инклюда нескольких базовых файлов
		LocalPlayer():ChatPrint(IGS.GetPhrase("autodonateinstalledincorrectly"))
		return
	end
	...
	uigs.Create("igs_button", function(self)
		...
		self:SetSize(150,27)
		self:UPDBalance()
		self:SetTooltip(IGS.GetPhrase("openpurchaselist"))
		...
		add:SetSize(27,27)
		add:SetText("+")
		add:SetTooltip(IGS.GetPhrase("topupbal"))
		add:SetActive(true)
