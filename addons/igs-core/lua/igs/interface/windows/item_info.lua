...
local function purchase(ITEM, buy_button)
	IGS.Purchase(ITEM:UID(), function(errMsg,dbID)
		...
		if errMsg then
			IGS.ShowNotify(errMsg, IGS.GetPhrase("purchaseerr"))
		...
		if ITEM:IsStackable() then
			buy_button:SetText(IGS.GetPhrase("bought") .. " " .. buy_button.purchased .. " " .. IGS.GetPhrase("pcs"))
		else
			if IsValid(m) then
				m:Close()
			end
			...
			if not IGS.C.Inv_Enabled then
				IGS.ShowNotify(IGS.GetPhrase("thanksforbuying"), IGS.GetPhrase("successfulbuying"))
				return
			end

			IGS.BoolRequest(IGS.GetPhrase("successfulbuying"),
				IGS.GetPhrase("thanksforpurchaseinv"),
			function(yes)
			...
			buy.DoClick = function(s)
				if not s:IsActive() then
					...
					IGS.BoolRequest(
						...
						(IGS.GetPhrase("notenoughdeposit")):format( PL_MONEY(need), ITEM:Name()),
						function(yes)
							if yes then
