...
hook.Add("IGS.CatchActivities","main",function(activity,sidebar)
	...
	local bg = sidebar:AddPage(IGS.GetPhrase("lastpurchases"))
	...
	---------------------------------------------------------------------------]]
	bg.tags:AddTag(IGS.GetPhrase("filter_reset"), function() bg.categs:Clear() addItems() end)
		:SetActive(true)
	...
	IGS.GetLatestPurchases(function(latest_purchases)
		...
		local function addPurchasePanel(v)
			...
			pnl:AddRow(IGS.GetPhrase("whobuyed") .. " ",v.nick or "NoName")
			if IGS.SERVERS.TOTAL > 1 then
				pnl:AddRow(IGS.GetPhrase("buyedon") .. " ",IGS.ServerName(v.server))
			-- else
			-- 	pnl:AddRow("UID: ",v.item)
			end
			pnl:AddRow(IGS.GetPhrase("buyeduntil") .. " ",IGS.TimestampToDate(v.expire) or IGS.GetPhrase("buyedforever"))
	...
	activity:AddTab(IGS.GetPhrase("service"),bg,"materials/icons/fa32/rub.png",etoGlavnayaVkladkaBlya)
end)
