local function loadTab(activity,sidebar,dat)
	local bg = sidebar:AddPage(IGS.GetPhrase("itemactions"))
	IGS.AddTextBlock(bg.side, nil, #dat > 0 and
		IGS.GetPhrase("textblock1") or IGS.GetPhrase("textblocklong")
	)
	...
	local infpan = uigs.Create("igs_iteminfo", function(p)
		...
		p:SetName("")
		p:SetDescription(IGS.GetPhrase("herewillbepurchaseinfo"))
	end, bg)
	...
	IGS.AddTextBlock(scr, IGS.GetPhrase("yourinv"), IGS.GetPhrase("itsemptyhere"))
	...
	scr:AddItem( uigs.Create("DIconLayout", function(icons)
		...
		function icons:AddItem(ITEM, dbID)
			...
			item:SetSign(IGS.GetPhrase("validuntil") .. " " .. IGS.TermToStr(ITEM:Term()))
			item.DoClick = function()
				...
				if IGS.C.Inv_AllowDrop then
					IGS.AddButton(bg.side,IGS.GetPhrase("drop"),function()
