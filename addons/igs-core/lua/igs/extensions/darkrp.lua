...
function STORE_ITEM:SetDarkRPMoney(iSum)
	self:SetDescription(IGS.GetPhrase("cmd_desc"):format(string.Comma(iSum)))
...
hook.Add("canBuyShipment", "IGS", function(pl, tItem)
	...
	if ITEM ~= nil then -- донатный итем
		...
		return allow or tobool(ITEM), false, message or IGS.GetPhrase("its_for_donaters")
	end
end)
...
hook.Add("playerCanChangeTeam", "IGS", function(pl, iTeam, bForce)
	...
	if ITEM ~= nil then -- донатный итем
		...
		return allow or tobool(ITEM), message or IGS.GetPhrase("its_for_donaters")
	end
end)
