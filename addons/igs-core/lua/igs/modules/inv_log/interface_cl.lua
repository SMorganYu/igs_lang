...
actions[1] = IGS.GetPhrase("purchase")
actions[2] = IGS.GetPhrase("activation")
actions[3] = IGS.GetPhrase("fall")
actions[4] = IGS.GetPhrase("peak")
...
function IGS.WIN.InvLog()
	return uigs.Create("igs_frame", function(bg)
		bg:SetTitle(IGS.GetPhrase("inventory_actions"))
		bg:SetSize(800, 600)
		bg:Center()
		bg:MakePopup()
		...
		function bg:AddLine(sOwner, sInfli, sItem, sAction, sDate, r)
			...
			line:SetTooltip(IGS.GetPhrase("operation_id"):format(r.action_id))
			...
			line.DoClick = function()
				...
				m:AddOption(IGS.GetPhrase("copyownersid"),function() SetClipboardText(r.owner) end)
				m:AddOption(IGS.GetPhrase("copyperformersid"),function() SetClipboardText(r.inflictor) end)
				m:AddOption(IGS.GetPhrase("player_actions"),function() bg:SearchSteamID(r.owner) end)
				m:AddOption(IGS.GetPhrase("actions_with"):format(sItem),function() bg:SearchGiftUID(r.gift_uid) end)
				if sAction == IGS.GetPhrase("activation") then -- а таких записях написан ID покупки, а не гифта (аве шиткодинг!)
					m:AddOption(IGS.GetPhrase("disable"), function()
		...
		bg.table = uigs.Create("igs_table", function(pnl)
			...
			pnl:SetTitle(IGS.GetPhrase("actions"))

			pnl:AddColumn(IGS.GetPhrase("owner"),120)
			pnl:AddColumn(IGS.GetPhrase("performer"),120)
			pnl:AddColumn(IGS.GetPhrase("item"))
			pnl:AddColumn(IGS.GetPhrase("gift_id"), 65)
			pnl:AddColumn(IGS.GetPhrase("action"),110)
		...
		local entry = uigs.Create("DTextEntry", function(self)
			self:Dock(LEFT)
			self:SetWide(200)
			self:SetValue(IGS.GetPhrase("sid_or_uid_item"))
		...
		uigs.Create("igs_button", function(self)
			...
			self:SetText(IGS.GetPhrase("find"))
		...
		bg.load = uigs.Create("igs_button", function(self)
			...
			self.UpdateLoaded = function(_, iLoaded, iTotal)
				if iLoaded == iTotal then
					self:SetText(IGS.GetPhrase("all_loaded"):format(iTotal))
					self:SetActive(false)
				else
					self:SetText(IGS.GetPhrase("load_more"):format(iLoaded, iTotal))
					self:SetActive(true)
				end
			end
		end, bottom)
